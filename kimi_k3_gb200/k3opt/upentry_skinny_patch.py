...
"""k3opt "upentry" copy of vLLM's fused_add_multicast_skinny_gemm.py (v0.30.0, md5 86833e39): identical except
griddepcontrol.launch_dependents moves from the end of the kernel (after the epilogue stores) to kernel entry.
...
class FusedAddMulticastSkinnyGemmUpEntry:
    ...
    def kernel(
        ...
        # k3opt upentry: trigger the PDL dependents at kernel ENTRY (vLLM: after the epilogue stores).
        # The Lamport consumer that follows polls the mailbox for readiness; safety: upentry_patch.py docstring.
...
_COMPILED: dict[tuple[object, ...], object] = {}  # upentry variant cache
...
def compile_kernel(
    *,
    ...
    compiled = cute.compile(
        FusedAddMulticastSkinnyGemmUpEntry(
            num_rows=num_rows,
